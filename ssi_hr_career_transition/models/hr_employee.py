...
class HrEmployee(models.Model):
    ...
    manual_date_terminate = fields.Date(
        string="Manual Date Terminate"
    )
    date_terminate = fields.Date(
        string="Date Terminate",
        compute="_compute_date_terminate",
        store=True
    )
    manual_date_permanent = fields.Date(
        string="Manual Date Permanent"
    )
    date_permanent = fields.Date(
        string="Date Permanent",
        compute="_compute_date_permanent",
        store=True
    )

    @api.depends('career_transition_ids', 'career_transition_ids.state', 'career_transition_ids.type_id')
    def _compute_career_transition(self):
        for record in self:
            ...
            if len(record.career_transition_ids) > 0:
                latest = record.career_transition_ids[0]
            ...
            if len(permanents) > 0:
                permanent = permanents[0]

    @api.depends('work_information_method', 'latest_career_transition_id', 'manual_company_id')
    def _compute_company_id(self):
        for record in self:
            record.company_id = record.manual_company_id

            if record.work_information_method == 'career_transition' and record.latest_career_transition_id:
                record.company_id = record.latest_career_transition_id.new_company_id

    @api.depends('work_information_method', 'latest_career_transition_id', 'manual_department_id')
    def _compute_department_id(self):
        for record in self:
            record.department_id = record.manual_department_id

            if record.work_information_method == 'career_transition' and record.latest_career_transition_id:
                record.department_id = record.latest_career_transition_id.new_department_id

    @api.depends('work_information_method', 'latest_career_transition_id', 'manual_job_id')
    ...
    @api.depends('work_information_method', 'latest_career_transition_id', 'manual_manager_id')
    ...
    @api.depends('work_information_method', 'latest_career_transition_id', 'manual_employment_status_id')
    def _compute_employment_status_id(self):
        for record in self:
            record.employment_status_id = record.manual_employment_status_id

            if record.work_information_method == 'career_transition' and record.latest_career_transition_id:
                record.employment_status_id = record.latest_career_transition_id.new_employment_status_id

    @api.depends('work_information_method', 'latest_career_transition_id', 'manual_date_join')
    def _compute_date_join(self):
        for record in self:
            record.date_join = record.manual_date_join

            if record.work_information_method == 'career_transition' and record.join_career_transition_id:
                record.date_join = record.join_career_transition_id.effective_date

    @api.depends('work_information_method', 'latest_career_transition_id', 'manual_date_terminate')
    def _compute_date_terminate(self):
        for record in self:
            record.date_terminate = record.manual_date_terminate

            if record.work_information_method == 'career_transition' and record.terminate_career_transition_id:
                record.date_terminate = record.terminate_career_transition_id.effective_date

    @api.depends('work_information_method', 'latest_career_transition_id', 'manual_date_permanent')
    def _compute_date_permanent(self):
        for record in self:
            record.date_permanent = record.manual_date_permanent

            if record.work_information_method == 'career_transition' and record.permanent_career_transition_id:
                record.date_permanent = record.permanent_career_transition_id.effective_date
